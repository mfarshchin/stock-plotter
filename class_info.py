...
class Stock(object):
    '''
    This class has functions to analyze and plot a stock
    '''
    ...
    #---------------------------------------------------------------------------- stock history
    def getHistory(self, days=365*5, interval='1d'):
    ...
    #---------------------------------------------------------------------------- volume analysis
    def Vol_Dist(self, df_d, bins=30, decay_halftime=0.3):
        df_d_vd = self.volume_distibution(df_d, bins=bins, decay_halftime=decay_halftime)
        return df_d_vd


    def volume_distibution(self, df, bins=30, decay_halftime=0.3):
        ...
        for _, row in stock_df.iterrows():
            ...
            acc_volume =self.accumulation(acc_volume, price_centers, volume, avg_point, low, high)
        return pd.DataFrame({'price_centers':price_centers,'volume_dist':acc_volume})


    def accumulation(self, acc_volume, price_centers, volume, avg_point, low, high):
    ...
    #---------------------------------------------------------------------------- moving average
    def mySimpleMA(self, tmp,spans=[5,10,21,60]):
        df_ma = tmp.copy()
        for days in spans:
    ...
    #---------------------------------------------------------------------------- price volume plot
    def plot_price_vol(self, df, df_v):
    ...
    #---------------------------------------------------------------------------- MA plot
    def plot_add_MA(self, df_ma, p, line_dash='solid'):
        tmp = df_ma.copy()
        # Average price line
    ...
    #---------------------------------------------------------------------------- purchase price
    def plot_add_purchasePrice(self, p, line_dash='solid'):
        if self.purchase_price != '':
            p.add_layout(Span(location=self.purchase_price, dimension='width', line_color='blue', line_width=1, line_dash=line_dash))
...
def history_plot(name, purchase_price='', ma=[50]):
    st = Stock(name, purchase_price)
    df_d = st.getHistory(days=365*5, interval='1d')
    df_d_vd = st.Vol_Dist(df_d, bins=100, decay_halftime=1)
    df_d_dailyMA = st.mySimpleMA(df_d,spans=ma)
    p = st.plot_price_vol(df_d, df_d_vd)
    p = st.plot_add_purchasePrice(p, line_dash='dashed')
    p = st.plot_add_MA(df_d_dailyMA, p)
    show(p)